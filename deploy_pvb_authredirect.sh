...
echo "Backing up..."
cp -r "$BACK" "$BACK.backup-authredirect-$(date +%F-%H%M)"
  ...
  try {
    ...
    if (result.rows.length === 0) {
      const insertResult = await db.query(
        ...
        [userinfo.sub, userinfo.email, userinfo.name || userinfo.preferred_username || userinfo.email, 'student']
      );
      user = insertResult.rows[0];
    } else {
      user = result.rows[0];
      await db.query(
        'UPDATE users SET email = $1, naam = $2, updated_at = NOW() WHERE authentik_id = $3',
        [userinfo.email, userinfo.name || userinfo.preferred_username || userinfo.email, userinfo.sub]
      );
    }
    req.session.user = {
      id: user.id,
